...
fn main() {
    ...
    io::stdin().read_line(&mut amqp_url).expect("Unable to read URL!");
    ...
    channel.queue_declare(queue_name, false, true, false, false, false, Table::new()).expect("Unable to declare queue!");

    println!("Type some text here below, it will be sent into the '{}' queue. Hit Enter when done.", queue_name);
    let mut input_data = String::new();
    io::stdin().read_line(&mut input_data).expect("Unable to read input data!");
